...
早稲田大学宇宙航空研究会WASA 鳥人間プロジェクトの引き継ぎ資料Wiki（MediaWiki）に対して、
自然言語で質問できるチャットボットです。
...
- 画面・履歴・アシスタントの現行仕様: [docs/03-フロントエンド.md](docs/03-フロントエンド.md)
...
次に、WikiとサイトのデータをWASA Chatが検索できる形へ取り込みます。
...
取り込みには少し時間がかかります。急いでいるときに戸惑わないよう、理由を書いておきます。

- `dump_wiki.py`は、さくらインターネットのレンタルサーバ上のWikiを叩きます。
  **相手のサーバーに負荷をかけないよう、リクエスト間に1秒の間隔を入れています。**
  そのため全ページの取得に数分かかりますが、これは意図的なものです。短くしないでください
- `dump_site.py`は、一般公開の公式サイト（wasa-birdman.com）を取り込みます

公式サイトも取り込んでいるのは、Wikiが「作り方」には詳しい一方で、
団体の成り立ちや歴代機体といった**対外的な説明**が薄いためです。
新入生や外部への説明を求められたときに答えられませんでした。

こちらも1秒間隔で、sitemapから519ページを辿ります（10分ほどかかります）。
なお`dump/site.jsonl`が無ければ`build_index.py`はWikiだけで索引を作るので、
省略しても動きます。
...
実際に使うとどう見えるかをまとめておきます。詳しい仕様は
[docs/03-フロントエンド.md](docs/03-フロントエンド.md)にあります。
...
- 使い方・注意事項・プライバシーポリシーは`/support.html`にあり、ログイン前でも読める
...
| 索引全体 | 646ページ / 1,003チャンク / 727,068字 |
| Wiki添付画像 | 67枚（本文参照57箇所） |

ファイルとしては`data/index.json`が3.2MB、`data/toc.md`が72KBです。
